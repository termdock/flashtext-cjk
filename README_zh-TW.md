...
這是一個維護中的 [FlashText](https://github.com/vi3k6i5/flashtext) 分支，專注於解決國際化 (i18n) 與 Unicode 相關問題，並透過 Rust 核心進行效能優化。
...
原始的 FlashText 專案已不再活躍維護，且在處理非英語語言時存在數個嚴重問題：

- **CJK 語言支援**: 無法正確提取相鄰的關鍵詞（中文、日文、韓文）。
- **Unicode 大小寫轉換**: 對於像土耳其語 `İ` 這樣的字符，會導致位置索引錯誤。
- **非 ASCII 邊界判定**: 對於國際字符的邊界處理存在多種邊緣情況。

本分支旨在修復這些問題，並保持與原版完全相容的 API。
...
**效能提升 (Performance)**
- **Rust Core**: 大幅提升吞吐量 (比 Python 快 3-4 倍)。
- **可擴展性**: 即使有 100k+ 關鍵詞，匹配時間幾乎保持常數。
- **相容性**: 與 Python API 100% 完全相容 (Drop-in replacement)。
